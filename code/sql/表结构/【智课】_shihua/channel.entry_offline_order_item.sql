-- =====================================================
-- 订单- 线下付款订单维度表 channel.entry_offline_order_item
-- =====================================================
--
-- 【表粒度】
...
-- =====================================================
CREATE TABLE
  `channel`.`entry_offline_order_item` (
    `id` string COMMENT 'id',
    `entry_id` bigint COMMENT '工单id',
    `is_school` boolean COMMENT '是否对应学校，true：是、false：否.',
    `school_code` bigint COMMENT '学校编号.',
    `buy_count` bigint COMMENT '采购数量.',
    `amount` double COMMENT '打款金额.',
    ...
    `school_region` string COMMENT '学校区域码',
    `agency_manager` string COMMENT '区域经理',
    `agency_manager_name` string COMMENT '区域经理名称',
    `operation_manager` string COMMENT '运营经理',
    `operation_manager_name` string COMMENT '运营经理名称',
    `super_vise` string COMMENT '大区经理',
    `super_vise_name` string COMMENT '大区经理名',
    `created_at` timestamp COMMENT '创建时间',
    `reality_amount` double COMMENT '业绩实际回款',
    `reality_amount_remark` string COMMENT '业绩实际回款计算方式',
    `created_user` string COMMENT '创建人',
    `reality_amount_at` timestamp COMMENT '填写业绩实际回款时间',
    `region_code` string COMMENT '区域码',
 ...
 ) COMMENT 'channel.entry_offline_order_item 表' ROW FORMAT SERDE 'org.apache.hadoop.hive.ql.io.orc.OrcSerde' STORED AS INPUTFORMAT 'org.apache.hadoop.hive.ql.io.orc.OrcInputFormat' OUTPUTFORMAT 'org.apache.hadoop.hive.ql.io.orc.OrcOutputFormat' LOCATION 'tos://yc-data-platform/user/hive/warehouse/channel.db/entry_offline_order_item'
...
-- `is_school`（列 COMMENT）：是否对应学校，true：是、false：否.
