-- =====================================================
-- 订单- 项目代付退款订单 channel.business_order_refund_extra_info
-- =====================================================
--
-- 【表粒度】
...
-- =====================================================
CREATE TABLE
  `channel`.`business_order_refund_extra_info` (
    `id` string COMMENT 'undefined',
    `order_id` string COMMENT '外键-订单id',
    `refund_amount` string COMMENT 'undefined',
    `transfer_pay` string COMMENT '退款-消耗购课款转入-金额',
    `rechange_pay` string COMMENT '退款-消耗充值-金额',
    `whether_recalled` boolean COMMENT '是否收回权益信息',
    `created_date` timestamp COMMENT '这里记录的创建时间',
    `refunded_time` timestamp COMMENT '退款时间',
    `updated_date` timestamp COMMENT '这里记录的更新时间'
 ) COMMENT 'channel.business_order_refund_extra_info 表' ROW FORMAT SERDE 'org.apache.hadoop.hive.ql.io.orc.OrcSerde' STORED AS INPUTFORMAT 'org.apache.hadoop.hive.ql.io.orc.OrcInputFormat' OUTPUTFORMAT 'org.apache.hadoop.hive.ql.io.orc.OrcOutputFormat' LOCATION 'tos://yc-data-platform/user/hive/warehouse/channel.db/business_order_refund_extra_info'
...
-- `whether_recalled`（列 COMMENT）：是否收回权益信息
--
