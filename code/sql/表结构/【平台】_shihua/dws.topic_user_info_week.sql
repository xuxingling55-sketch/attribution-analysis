-- =====================================================
-- 公用- 用户宽周表 dws.topic_user_info_week
-- =====================================================
--
-- 【表粒度】
...
--
-- 【常用关联】
--   - 与 `dws.topic_user_info`、`dws.topic_user_info_month`、`dw.dim_user` 按 `u_user` 对齐
--
-- 【常用筛选条件】
--   场景条件：
--   - is_test_user 等按需求
...
-- =====================================================
--
-- > 按 glossary【平台】规则 R34：本表为 **DWS** 用户主题周表；列集与 `dws.topic_user_info` 周粒度快照一致（无 `*_month` 后缀列）。
-- > **同名字段**枚举与含义 **继承** `dws.topic_user_info.sql` 第三段「枚举值」（含 `business_user_pay_status_*`、`user_allocation`、`active_type` 等以 COMMENT 与上游加工为准的字段）。
