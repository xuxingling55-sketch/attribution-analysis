-- =====================================================
-- 公用- 用户活跃月表 dws.topic_user_active_detail_month
-- =====================================================
--
-- 【表粒度】
-- 一个用户一个下载渠道一个产品id一个活跃端口一个手机品牌一个手机型号一个sn_code一条记录，按自然月汇总（分区 month int，格式以库内约定为准）；含 `mid_grade`（当月第一次活跃时）、`user_strategy_*_month`；本表无日表中的 `attribution`（仅注册口径）
--
-- 【业务定位】
--   - 【归属】公用 / 用户活跃月表。
-- - 与日表 `dws.topic_user_active_detail_day` 同族：月粒度行为汇总；全量/C 端学习行为叠加 `product_id`、`client_os`、`active_user_attribution`（见 glossary「C 端活跃默认筛选」）
--   - 寒假/大盘流量仍以日表 `is_active_user`、`is_test_user` 为主；月表侧重周期内汇总（活跃天数、次数、付费、电销外呼等）
-- - 同族：`topic_user_active_detail_week`（周分区、`user_strategy_*_week`）；策略字段本表为 `user_strategy_tag_month`、`user_strategy_eligibility_month`
--
-- 【统计口径】
--   周期内汇总类指标（*_cnt、duration、`active_day_cnt` 等）见字段 COMMENT；UV 类按分析场景对 `u_user` 去重
--
-- 【常用关联】
--   - 与日表 `dws.topic_user_active_detail_day` 同族：`u_user` + `month` 分区与 `dw.dim_date`/`month` 业务键关联（与日表对齐时筛选口径见【常用筛选条件】）
--
-- 【常用筛选条件】
...
-- 【注意事项】
--   - 更新频率：月更（以调度为准）
--   - 数据来源：`code/sql/临时文件/dws.topic_user_active_detail_month.md`（若与线上一致则以库为准）
--   - 知识库约定：取数与分析仅使用 business_user_pay_status_*；user_pay_status_*（无 business_ 前缀）列不在知识库维护口径

CREATE EXTERNAL TABLE `dws`.`topic_user_active_detail_month` (
 ...
 `business_user_pay_status_business` string COMMENT '大会员付费用户(统计日期之前买过大会员商品)、续费用户(统计日期之前买过正价课)、新用户(统计日期30天内注册的)、老用户(统计日期30以前注册的)',
  ...
  `mid_grade` string COMMENT '年级(当月第一次活跃时)'
)
...
LOCATION 'tos://yc-data-platform/user/hive/warehouse/dws.db/topic_user_active_detail_month'

-- =====================================================
-- 枚举值
-- =====================================================
--
-- 以下取值含义与 `dws.topic_user_active_detail_day` 第三段对齐（同一业务枚举）；本表策略字段为 `user_strategy_tag_month`、`user_strategy_eligibility_month`；`mid_grade` 为当月第一次活跃时年级（见字段 COMMENT）。`business_user_pay_status_*` 若与字段 COMMENT「大会员」表述不一致，以落表为准。本表无 `attribution`、`is_admin_user` 列，相关含义见日表枚举。
--
-- ## role（身份）
--
-- | 枚举值 | 含义 |
...
-- | 高净值用户 | 统计日期之前方案型商品（不包括商品二级分类 id 为一年积木块、体验机、到期型培优课积木块等）（见 `dws.topic_user_info` 字段 COMMENT） |
--
-- ## user_strategy_tag_month（用户策略标签）
--
-- | 枚举值 | 含义 |
-- |--------|------|
...
--
-- ## user_strategy_eligibility_month（用户策略资格）
--
-- | 枚举值 | 含义 |
-- |--------|------|
