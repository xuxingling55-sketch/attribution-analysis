...
-- 新品销量表现 - 每周销量趋势（供报告折线图：周 × 各新品订单量）
-- 近一个月，按「上周五至本周四」为一周（weekofyear(date+3天）），使用日历表 cross join 保证各新品周起始日一致且不缺失
...
types_weeks as (
  select t.types, w.dt
  from (select distinct types from new_good_types) t
...
select
  ...
  , orders                                                      -- 当周订单量
from agg
order by week_start_date, types
limit 100000
