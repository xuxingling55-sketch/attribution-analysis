...
<parameter> 
and `business_gmv_attribution` in ('${doris_business_gmv_attribution_month}')
</parameter>
...
,a.order_id
,b.monthdiff
,sum(case when datediff(c.paid_time_sk,a.paid_time_sk) <= b.monthdiff * 31 then c.amount else 0 end) as xufei_amount
,count(distinct case when datediff(c.paid_time_sk,a.paid_time_sk) <= b.monthdiff * 31 then c.u_user end) as xufei_users
,sum(case when datediff(c.paid_time_sk,a.paid_time_sk) <= b.monthdiff * 31 and c.is_zuhe = '组合品' then c.amount end) as zuhe_xufei_amount
,count(distinct case when datediff(c.paid_time_sk,a.paid_time_sk) <= b.monthdiff * 31 and c.is_zuhe = '组合品' then c.u_user end) as zuhe_xufei_users
,sum(case when datediff(c.paid_time_sk,a.paid_time_sk) <= b.monthdiff * 31 and c.is_zuhe = '非组合品' then c.amount end) as unzuhe_xufei_amount
,count(distinct case when datediff(c.paid_time_sk,a.paid_time_sk) <= b.monthdiff * 31 and c.is_zuhe = '非组合品' then c.u_user end) as unzuhe_xufei_users
from orders a 
cross join (
SELECT 1 AS monthdiff UNION ALL SELECT 2 UNION ALL SELECT 3 -- select 0 是当天的分组，这里不要
...
-- where ceil(datediff(cast(replace(date(now()),'-','') as int),a.paid_time_sk)/31)  >= b.monthdiff 
...
,old_type
,monthdiff
,max(ceil(datediff(cast(replace(date(now()),'-','') as int),paid_time_sk)/31)) as max_month
