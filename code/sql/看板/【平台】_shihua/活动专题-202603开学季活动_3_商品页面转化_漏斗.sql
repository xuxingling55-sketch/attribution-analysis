...
-- 业务域：【平台】_shihua
-- 图表/组件：活动专题-202603开学季活动_3_商品页面转化_漏斗
...
select 
'活跃人数' as type
,count(distinct a.u_user) users
,count(distinct a.u_user) / count(distinct a.u_user) per
...
select 
'进入人数' as type
,count(distinct b.u_user) users
,count(distinct b.u_user) / count(distinct a.u_user) per
from act_user_app_c a 
left join yure_page b 
on a.u_user=b.u_user and a.day=b.day 
left join yure_order c 
...
select 
'付费人数' as type
,count(distinct c.u_user) users
,count(distinct c.u_user) / count(distinct b.u_user) per
from act_user_app_c a 
left join yure_page b 
on a.u_user=b.u_user and a.day=b.day 
left join yure_order c 
