...
-- 业务域：【平台】_shihua
-- 图表/组件：202511_线索品198六期_整体转化累计_对照组合并
...
select
    ab_name as `实验分组`
    ,case
        when ab_name = '3-对照组' then to_date('2024-01-01')
        when ab_name = '4-left组' then to_date('2024-01-01')
        else to_date('2025-01-01')
    end as date_type
    ,case
        when ab_name = '3-对照组' then '对照组'
    ...
    ,good_type as `商品分类`
    ,count(distinct if(amount > 0, order_id, null)) as `订单量`
    ,sum(amount) as `付费金额`
    ,sum(pb_amount) as `方案型商品付费金额`
    ,sum(non_pb_amount) as `常规商品付费金额`
    -- rank_total：周期内用户首单/续单；金额与「付费金额」可加总一致
    ...
    ,count(distinct if(amount > 0 and rank_total = 1, pay_user, null)) as `首购付费人数` -- 周期内至少一笔首单的去重用户（可与续购重叠）
    ,count(distinct if(zuhe_amount > 0 and rank_total = 1, pay_user, null)) as `组合品首购人数`
    ,count(distinct if(qita_amount > 0 and rank_total = 1, pay_user, null)) as `非组合品首购人数`
    ,count(distinct if(amount > 0 and rank_total > 1, pay_user, null)) as `续费人数`
    ,count(distinct if(zuhe_amount > 0 and rank_total > 1, pay_user, null)) as `组合品续费人数`
    ,count(distinct if(qita_amount > 0 and rank_total > 1, pay_user, null)) as `非组合品续费人数`
    ,count(distinct case when rn2_total = 1 then u_user end) as `分组活跃人数` -- 汇总：去重活跃人数
    ,count(distinct case when rn3_total = 1 then page_user end) as `商品页面进入人数`
    ,count(distinct case when rn3_total = 1 and non_pb_cnt > 0 then page_user end) as `常规商品页面进入人数`
    ,count(distinct case when rn3_total = 1 and pb_cnt > 0 then page_user end) as `方案型商品页面进入人数`
    ,count(distinct case when rn1_allpay_total = 1 and amount > 0 then pay_user end) as `付费人数`
    ,count(distinct case when rn1_allpay_total = 1 and pb_amount > 0 then pay_user end) as `方案型商品付费人数`
    ,count(distinct case when rn1_allpay_total = 1 and non_pb_amount > 0 then pay_user end) as `常规商品付费人数`
from r4
group by
    1, 2, 3, 4, 5, 6, 7, 8, 9
