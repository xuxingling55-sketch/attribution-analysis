...
-- 新品销量表现 - 按周×渠道汇总（周趋势下钻，仅「从小学物理」）
-- 文件名后缀 _从小学物理 标明为特定新品下钻，避免与「全新品」渠道口径混淆
-- 近一个月，周口径与「新品销量表现_周趋势」一致，使用日历表 cross join；筛选 新品=从小学物理
...
new_good_types as (
  select
    ...
    case
      when sell_from regexp 'telesale' then '电销/网销'
...
-- 仅保留「从小学物理」用于渠道下钻
congxiaoxue_only as (
  select sellfrom, order_id, dt
  from new_good_types
  where types = '从小学物理'
),
channels_weeks as (
  select c.sellfrom, w.dt
  from (select distinct sellfrom from congxiaoxue_only) c
  cross join week_day w
),
base as (
  select
    cw.sellfrom,
    cw.dt,
    n.order_id
  from channels_weeks cw
  left join congxiaoxue_only n
    on cw.sellfrom = n.sellfrom
   and cw.dt = n.dt
),
agg as (
  select
...
select
  ...
  , orders                                                        -- 当周订单量
from agg
order by week_start_date, sellfrom
limit 100000
