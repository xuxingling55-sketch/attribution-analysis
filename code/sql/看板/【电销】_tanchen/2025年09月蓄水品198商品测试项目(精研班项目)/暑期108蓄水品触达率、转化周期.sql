with y1 as 
(
  select distinct substr(paid_time,1,10) paid_time,'' as ab_name,good_name as good_type,u_user
  from dws.topic_order_detail
  where substr(paid_time,1,10) between '2025-06-03' and '2025-06-22'
  and sku_group_good_id in ('2ad36071-17ec-4eda-9a7a-27c005fd61fa','10138aa5-ea9c-4723-9ac7-4aab637e7218')
) --同一天一个商品多次购买算一次
...
(
  ...
  (
    ...
    (
        ...
        from aws.clue_info
        where substr(clue_expire_time,1,10)>='2025-06-03' and substr(created_at,1,10)<= '2025-07-05'
    ) y2 --电销触达信息
...
(
  ...
  where
  substr(pay_time,1,10)  between '2025-06-03' and '2025-07-05'
  and worker_id <> 0
  and in_salary = 1
  and is_test = false
  and status = '支付成功'
