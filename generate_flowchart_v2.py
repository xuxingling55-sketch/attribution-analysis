...
Q3: 全量vs局部判断 → 多维对比（年级×渠道/用户分层/渠道间） → 专项深挖
"""
...
lines.append(txt(CX, y + 28,
                 'Q2 结论：数据真实，GMV -4.8% 超出业务方确认的正常范围 [-4%, +4%]，付费用户 -8.4%，进入 Q3 下钻 ✅',
...
y += 88
lines.append(rect(PAD, y, W - PAD * 2, 450, 'white', stroke='#FED7E2', sw=2))
...
for ri, (h, sql, res, contrib, judge, col) in enumerate([
    ('H1 用户规模萎缩（Q3-D确认）',
     ...
     '付费用户 5,342→4,891（↓8.4%），超出历史波动范围，显著', '~55%', '🔴 核心主因', '#C53030'),
    ('H2 商品结构（Q3-A确认）',
     'good_name 维度，升学规划课集中下跌',
     '[小升初/四升五/七升八]三类课型占跌幅65%', '~40%', '🔴 核心主因', '#C53030'),
    ('H3 渠道效率（Q3-B确认，订单表）',
     'business_gmv_attribution，商业化 vs 电销对比',
     '订单表：商业化-14.3%，电商-33.4%（局部跌）', '~60%', '🔴 渠道主因', '#C53030'),
    ('H3 渠道效率（Q3-B补充，crm表）',
     '⚠️ crm表口径独立，贡献度仅在电销内部可比，不与订单表加总',
     'crm表：电销团队整体+2.0%（逆势），印证局部判断', '—', '✅ 电销正常', '#276749'),
    ('H4 价格/AOV 变化',
     'SUM(sub_amount)/COUNT(DISTINCT u_user)',
     'AOV: 1,008→1,006（-0.2%），不影响趋势', '~0%', '✅ 已排除', '#276749'),
    ('H5/H6 外部竞品/APP技术',
     'Q3 无信号，跳过',
     '竞品无大促；iOS漏斗暂缺，可单独补核', '—', '— 跳过', '#888'),
]):
    ry = th5_y + 40 + ri * 50
    if ri % 2 == 0:
...
lines.append(rect(PAD + 12, y + 408, W - PAD * 2 - 24, 32, '#FFF5F7'))
lines.append(txt(CX, y + 430,
                 "SQL 规范：product_id='01' | is_test_user=0 | sub_amount≥39 | mid_grade | 订单表 paid_time | 电销表 pay_time | 电销 P99 过滤 | crm贡献度独立计算",
