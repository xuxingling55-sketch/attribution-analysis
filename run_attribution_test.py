...
if dev > 50:
    print(f'  ⚠️  日均偏差 {dev:.1f}% > 50%，疑似数据延迟或分区未刷新。')
    print()
    print('  【数据质检提示 - 请业务侧确认】')
    print('  当前周的每日订单量与近四周同期均值偏差超过 50%，可能原因：')
    print('    1. 数据仓库分区尚未刷入（常见于当天 / 次日早）')
    print('    2. 当前周存在大促、节假日或业务口径调整')
    print('    3. 真实业务量出现结构性突变')
    print()
    print('  ❓ 是否在上述情况下仍希望继续归因？')
    print('    - 如果是数据延迟：建议等数据刷新后重跑，结论更可靠。')
    print('    - 如果有业务原因（大促/节假日）：请告知，可切换基准或加注标注继续分析。')
    print('    - 如果仍需立即归因：请回复"继续"，系统将在报告中标注[数据质量存疑]。')
    print()
    print('  脚本已暂停，等待业务侧确认。如需强制继续，请在命令行添加 --force 参数重新运行。')
    sys.exit(2)   # exit code 2 = 需要用户确认，区别于 exit(1) 的真实错误
else:
    print('  ✅ 分区完整，数据量级正常')
